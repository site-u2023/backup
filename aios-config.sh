...
select_language() {
    while true; do
        ...
        # 入力内容をデバッグ表示
        echo "You entered: '$LANGUAGE_CHOICE'"  # デバッグ表示（入力内容をシングルクォーテーションで囲んで表示）
        
        # 入力をトリム
        LANGUAGE_CHOICE=$(echo "$LANGUAGE_CHOICE" | sed 's/[[:space:]]//g')

        # 入力内容確認
        echo "Trimmed input: '$LANGUAGE_CHOICE'"  # トリム後の内容を表示
