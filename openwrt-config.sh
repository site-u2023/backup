...
display_system_info() {
    ...
    if [ "$lang" = "en" ]; then
        ...
        echo -e "$(color "white" "Country: BASE_DIR="${BASE_DIR:-/tmp/aios}"
SELECTED_LANGUAGE=$(cat "${BASE_DIR}/check_language") && echo "$(sh ${BASE_DIR}/country-zonename.sh "$SELECTED_LANGUAGE")"")"
        ...
        echo -e "$(color "white" "カントリー: BASE_DIR="${BASE_DIR:-/tmp/aios}"
SELECTED_LANGUAGE=$(cat "${BASE_DIR}/check_language") && echo "$(sh ${BASE_DIR}/country-zonename.sh "$SELECTED_LANGUAGE")"")"
