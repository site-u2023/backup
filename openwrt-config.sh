...
main_menu() {
    ...
    MENU1 MENU2 MENU3 MENU4 MENU5 MENU6 MENU00 MENU01 SELECT1
    ACTION1 ACTION2 ACTION3 ACTION4 ACTION5 ACTION6 ACTION00 ACTION01
    TARGET1 TARGET2 TARGET3 TARGET4 TARGET5 TARGET6 TARGET00 TARGET01
    option=""
...
display_system_info() {
    lang="${SELECTED_LANGUAGE:-en}"
    available_memory=$(free | awk '/Mem:/ { print int($4 / 1024) }')
    available_flash=$(df | awk '/overlayfs:\/overlay/ { print int($4 / 1024) }')
    usb_devices=$(ls /sys/bus/usb/devices | grep -q usb && echo "Detected" || echo "Not detected")

    # 言語ごとのメッセージを定義
    case "$lang" in
        "en"|"ja")
            echo -e "$(color "white" "Memory: ${available_memory} MB")"
            echo -e "$(color "white" "Flash Storage: ${available_flash} MB")"
