...
display_system_info() {
    ...
    if [ "$lang" = "en" ]; then
        ...
        echo -e "$(color "white" "Country:  COUNTRY="$(sh ${BASE_DIR}/country-zonename.sh "$(cat ${BASE_DIR}/check_country)")")"
        ...
        echo -e "$(color "white" "カントリー:  COUNTRY="$(sh ${BASE_DIR}/country-zonename.sh "$(cat ${BASE_DIR}/check_country)")")"
